...
<script>
import JurnalHService from "../../services/jurnal_harian.service";
...
export default {
  ...
  methods: {
    ...
    showModalEdit(jurnal) {
      this.statusmodal = true;
      $("#modalmuncul").modal("show");
      this.getAbsensiById(jurnal);
    },
    getdata() {
      this.$Progress.start();
      JurnalHService.getJurnalH()
        .then((response) => {
          this.jurnalh = response.data;
          $(function () {
            $("#myTable")
              .DataTable({
                responsive: false,
    ...
    getJurnalHById(id) {
      JurnalHService.getJurnalHById(id)
        .then((response) => {
    ...
    simpanData() {
      ...
      this.loading = true;
      this.disabled = true;
      JurnalHService.createJurnalH(this.form)
        .then((response) => {
    ...
    updateData() {
      ...
      this.loading = true;
      this.disabled = true;
      JurnalHService.updateJurnalH(this.form.id, this.form)
        .then((response) => {
          this.from = response.data;
          console.log(response.data);
          this.getdata();
    ...
    deleteData(id) {
      Swal.fire({
        ...
        if (result.value) {
          JurnalHService.deleteJurnalH(id)
            .then((response) => {
    ...
    resetForm() {
      (this.form.siswaId = ""),
      (this.form.tgl_jurnal = ""),
      (this.form.jam_jurnal = ""),
      (this.form.isi_jurnal = "");
    },
  },
};
</script>
